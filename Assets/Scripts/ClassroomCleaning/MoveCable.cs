...
//		Debug.Log(IDBoje.ToString() + cable.CableCol);
	...
	void OnMouseDown(){
		...
		if(!cable.bIskoriscen &&  !bDrag && bEnableMove && !MiniGameConnectWires.bPause )
		{
			bDrag = true;
...
//				Debug.Log(allWiresColl[i].transform.name);
			...
			if(bTestWire) 
			{
				...
				MiniGameConnectWires.ConnectedWires = -1;
	...
	void OnTriggerStay2D_manual(Collider2D other) 
	{
		if( other.tag == "Wire")
		{
			...
			if(    (int)mc.CableCol == IDBoje)
			{
				...
				MiniGameConnectWires.ConnectedWires++;
				MiniGameConnectWires.ClickedIdBoje =-1;
