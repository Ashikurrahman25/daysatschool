...
public class ItemDrag : MonoBehaviour,IBeginDragHandler, IDragHandler, IEndDragHandler {
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        tempVector.z = 0;
        if (tempVector.y+Offset.y > topLimit.y)
            tempVector.y = topLimit.y-Offset.y;
        targetMovePosition = tempVector;

        if (lepring)
        {
            transform.position = Vector3.Lerp(transform.position, (targetMovePosition+Offset), draggingSpeed * Time.fixedTime / 1.5f);
            if (Vector3.Distance(transform.localPosition, targetMovePosition) < 0.2f)
            {
                transform.position = (targetMovePosition + Offset);
            }
        }
        else
        {
...
//            Debug.Log("Target enum: "+currentTarget.name + " " + currentTarget.effectTriggerEnum);
    ...
    IEnumerator CTriggerStay(TargetConditionBase target)
    {
        while (onTargetPlace)
        {
            ...
            yield return new WaitForFixedUpdate(); //TODO TriggerStay fire rate
        }
        yield return null;
    }
    ...
    IEnumerator CRub(TargetConditionBase target)
    {
        while (onTargetPlace)
        {
            ...
            if (Vector3.SqrMagnitude(lastFramePointerPos - currentPointerPos) > 0.5f)    //TODO Rub Distance trashold
            {   
                target.GetComponent<TargetConditionBase>().TargetCondition();
                Debug.Log("Rubbing: " + target.name);
            }
            lastFramePointerPos = currentPointerPos;
            yield return new WaitForFixedUpdate(); //TODO Rub fire rate
        }
    }
...
//        Debug.Log("Disconeted");
    ...
    public void StopBackToStartPos()
    {
        StopCoroutine(LerpToTargetPos(startPos));
    }
