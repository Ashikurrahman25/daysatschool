...
//		}
		...
		public static Texture2D ConvertSpriteToTexture2DForBmp(Sprite sr)
		{

			//sr.sprite = patterns [index];
			Texture2D texTex = new Texture2D ((int)sr.rect.size.x,(int) sr.rect.size.y, TextureFormat.ARGB32, false);
			Color[] tmp = sr.texture.GetPixels((int)sr.rect.x, 
			                                   (int)sr.rect.y, 
			                                   (int)sr.rect.size.x, 
			                                   (int)sr.rect.size.y,0 );
