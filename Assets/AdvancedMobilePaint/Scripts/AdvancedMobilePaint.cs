...
public class AdvancedMobilePaint : MonoBehaviour{
		...
		private Vector2 pixelUVStamp = new Vector2 (5000,5000);
//		private bool stampDrawn = false;
		...
		// FIXME Cackano!
		public bool drawIntended = false;  // if player is trying to draw. this is evaluated before and after zooming
		public bool floodFillDrawn = false;
	...
	public void DrawOnPointerUp()	//TODO TODO TODO TODO TODO TODO mousepaint3
	{
            if(ArtSchoolGameManager.Instance!=null)
                ArtSchoolGameManager.Instance.Painted();
			if(drawMode != DrawMode.FloodFill)return;
			if(!drawIntended) return;
			if(!drawEnabled)return;

            Debug.Log("Old PixelUV: " + pixelUV);
            if (!Physics.Raycast (useAlternativeRay?Camera.main.ScreenPointToRay(Camera.main.WorldToScreenPoint(raySource.position)):Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity)) 
			{
			...
			Vector3 forward = Camera.main.ScreenToWorldPoint(Input.mousePosition);
			Debug.DrawRay(forward + Vector3.forward, forward + Vector3.forward * 10, Color.red, 5f);
            ...
            Debug.Log("Updated PixelUV: " + pixelUV);

            Debug.Log("Hit alpha: " + maskPixels[((texWidth * ((int)pixelUV.y) + (int)pixelUV.x) * 4) + 3] + useMaskLayerOnly);
            if (useMaskLayerOnly && maskPixels[ ((texWidth*((int)pixelUV.y)+(int)pixelUV.x)*4) +3 ] == 255)
			{
                Debug.Log("Hit on black");
                return;
			}
			if(floodFillDrawn)
			{
                Debug.Log("Exited from flood fill");
                return;
			}
...
//			if(floodFillDrawn)return;
			...
			if(undoEnabled && drawMode != DrawMode.Stamp)	//FIXME Cackano! Za stamp mod se koordinate dodaju samo kada se iscrta stamp
			{
			...
			floodFillDrawn = false;
            if (Application.loadedLevelName != "TentAssembly")
            {
                drawIntended = false; //FIXME Cackano!
                drawEnabled = false;
            }

			Debug.LogError("DRAW Pointer UP");
	}
...
//				GameObject.Find("Canvas/Text1").GetComponent<Text>().text = "UV " + pixelUV.ToString();
	...
	//
		// MousePaint() modified for this game speciffic
		void MousePaint3 () 
		{
            Debug.Log("MOS 3 PAINT");
			if(drawMode == DrawMode.FloodFill)return;
            Debug.Log("MOS 3 PAINT PASSED FLOOD");
            ...
            {	if(ArtSchoolGameManager.Instance!=null)
				...
				if (useLockArea)
				{
					if (!Physics.Raycast (useAlternativeRay?Camera.main.ScreenPointToRay(Camera.main.WorldToScreenPoint(raySource.position)):Camera.main.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, paintLayerMask)) return;
					if(hit.collider!=gameObject.GetComponent<Collider>()) return;
					if(!drawIntended)return;	//FIXME Cackano
					//					Input.multiTouchEnabled=false; //FIXME Cackano
					CreateAreaLockMask((int)(hit.textureCoord.x*texWidth), (int)(hit.textureCoord.y*texHeight));
					//					Debug.Log("NAIL new step generated");
					if(undoEnabled)
					{
...
//							drawUndoStep.type=2;
            ...
            if (Input.GetMouseButton(0) && Input.touchCount<=1)
			{
				...
				case DrawMode.Pattern: // draw with pattern	
					...
					break;
				case DrawMode.CustomBrush: // custom brush
//					if(!useAdditiveColors && (pixelUVOld == pixelUV)) break;	//FIXME Cackano!
					BitmapBrushesTools.DrawCustomBrush2((int)pixelUV.x, (int)pixelUV.y,this);
					textureNeedsUpdate = true;
					break;	
				case DrawMode.FloodFill: // floodfill
...
//					textureNeedsUpdate = true;
					break;	
				case DrawMode.Stamp:	// FIXME Cackano!
//					Debug.Log("Distance: " + Vector2.Distance (pixelUV,pixelUVStamp));
					if(Vector2.Distance (pixelUV,pixelUVStamp)<= customBrushWidth)break;
					BitmapBrushesTools.DrawCustomBrush2((int)pixelUV.x, (int)pixelUV.y,this);

					pixelUVStamp = pixelUV;
...
//						drawUndoStep.type=2;
			...
			if (connectBrushStokes && Vector2.Distance(pixelUV,pixelUVOld)>(brushSize/2) && Input.touchCount<=1)
			{
				...
				case DrawMode.Stamp:
					#if UNITY_EDITOR
					BitmapBrushesTools.DrawStampLineWithBrush(pixelUVOld, pixelUV,this);
					#endif
					#if !UNITY_EDITOR 
					if(Input.touchCount==1)
					{
			...
			if (Input.GetMouseButtonUp(0)&& Input.touchCount<=1)
			{
//				floodFillDrawn = false;		//FIXME Cackano
				if(!drawIntended)return;	//FIXME Cackano
				//if (hideUIWhilePainting && !isUIVisible) ShowUI();
				if (drawEnabled && drawUndoStep!=null && undoEnabled) {
					UStep c= new UStep();
                ...
                if (Application.loadedLevelName != "ClassroomCleanUpScene")
                {
                    drawIntended = false; //FIXME Cackano!
                    drawEnabled = false;
                }
...
//		}
		...
		public void ReadMaskImage()
		{
            ...
            Debug.Log("mask: " + c.Length + " tex " + texWidth*texHeight);
			int pixel = 0;
...
//			Debug.Log (corners[3]);
			...
			Debug.LogError(gameObject.GetComponent<MeshCollider>().gameObject.layer);
		...
		public bool SetBitmapBrush(Texture2D brushTexture,BrushProperties brushType, bool isAditiveBrush,bool brushCanDrawOnBlack,Color brushColor, bool usesLockMasks,bool useBrushAlpha,Texture2D brushPattern )
		{
			...
			brushSize = customBrush.width; //FIXME cackano!
			ReadCurrentCustomBrush();
			brushMode=brushType;
		...
		/// <summary>
		/// Setuje teksturu po kojoj se crta tako sto postavlja samo crne i providne piksele.
		/// </summary>
		/// <returns><c>true</c>, if drawing texture was set, <c>false</c> otherwise.</returns>
		/// <param name="texture">Texture.</param>
		public bool SetMandalaTexture (Texture2D texture, FilterMode mode)
		{
			texWidth=texture.width;
			texHeight=texture.height;
			//support for NPOT
				...
				for(int j=0;j<texture.width;j++)
			{
				pixels[pix]=(byte)(texturePixels[i*texture.width+j].r*255);//R
				pixels[pix+1]=(byte)(texturePixels[i*texture.width+j].g*255);//G
				pixels[pix+2]=(byte)(texturePixels[i*texture.width+j].b*255);////
				pixels[pix+3]=(byte)(texturePixels[i*texture.width+j].a*255);//A

//				if(texturePixels[i*texture.width+j].r > 0.2f)	//FIXME cackano
//					pixels[pix] = 255;
//				else
//					pixels[pix] = 0;
//				if(texturePixels[i*texture.width+j].g > 0.2f)	//FIXME cackano
//					pixels[pix+1] = 255;
//				else
//					pixels[pix+1] = 0;
//				if(texturePixels[i*texture.width+j].b > 0.2f)	//FIXME cackano
//					pixels[pix+2] = 255;
//				else
//					pixels[pix+2] = 0;
//				if(texturePixels[i*texture.width+j].a > 0.5f)	//FIXME cackano
//					pixels[pix+3] = 255;
//				else
//					pixels[pix+3] = 0;
		...
		/// <returns><c>true</c>, if drawing mask was set, <c>false</c> otherwise.</returns>
		/// <param name="texture">Texture.</param>
		public Texture2D maskToShow;
		public bool SetMandalaMask(Texture2D texture, FilterMode mode)		
		{
			Color [] texturePixels=texture.GetPixels();
			maskToShow = new Texture2D(texture.width,texture.height,TextureFormat.RGBA32,false);
			maskToShow.filterMode = mode;
			maskPixels= new byte[texture.width*texture.height*4];
//			Byte[] maskToShowPixels= new byte[texture.width*texture.height*4];
			int pix=0;
			for(int i=0;i<texture.height;i++)
				for(int j=0;j<texture.width;j++)
			{
...
//				maskPixels[pix+3]=(byte)(texturePixels[i*texture.width+j].a*255);//A
				...
				if(texturePixels[i*texture.width+j].r > 0.3f)	//FIXME cackano
					maskPixels[pix] = 255;
				else
					maskPixels[pix] = 0;
				if(texturePixels[i*texture.width+j].g > 0.3f)	//FIXME cackano
					maskPixels[pix+1] = 255;
				else
					maskPixels[pix+1] = 0;
				if(texturePixels[i*texture.width+j].b > 0.3f)	//FIXME cackano
					maskPixels[pix+2] = 255;
				else
					maskPixels[pix+2] = 0;
				if(texturePixels[i*texture.width+j].a > 0.3f)	//FIXME cackano
					maskPixels[pix+3] = 255;
				else
					maskPixels[pix+3] = 0;

				//Mask To Show
//				if(texturePixels[i*texture.width+j].r > 0.9f)	//FIXME cackano
//					maskToShowPixels[pix] = 255;
//				else
//					maskToShowPixels[pix] = 0;
//				if(texturePixels[i*texture.width+j].g > 0.9f)	//FIXME cackano
//					maskToShowPixels[pix+1] = 255;
//				else
//					maskToShowPixels[pix+1] = 0;
//				if(texturePixels[i*texture.width+j].b > 0.9f)	//FIXME cackano
//					maskToShowPixels[pix+2] = 255;
//				else
//					maskToShowPixels[pix+2] = 0;
//				if(texturePixels[i*texture.width+j].a > 0.2f)	//FIXME cackano
//					maskToShowPixels[pix+3] = 255;
//				else
//					maskToShowPixels[pix+3] = 0;

//				if(texturePixels[i*texture.width+j].a > 0.9f)
//				{
//					maskPixels[pix] = 255;
//					maskPixels[pix+1] = 255;
//					maskPixels[pix+2] = 255;
...
//				}
			...
			maskToShow.LoadRawTextureData (maskPixels);
			maskToShow.Apply();
            if(transform.childCount > 0)
                transform.GetChild(0).GetComponent<RawImage>().texture = maskToShow;
	...
	//this is fix for Camping game
        public void EquilizePixelUVs()
        {
            pixelUVOld = pixelUV;
        }
	}
