...
//			int minDistance=(int)(paintEngine.brushSize>>1); // divide by 2, you might want to set mindistance to smaller value, to avoid gaps between brushes when moving fast
		...
		//FIXME Cackano - Spaja linije za Stamp mod
		public static void DrawStampLineWithBrush(Vector2 start, Vector2 end,AdvancedMobilePaint paintEngine)
		{
//			Color c = paintEngine.paintColor;
...
//			int minDistance=(int)(paintEngine.brushSize>>1); // divide by 2, you might want to set mindistance to smaller value, to avoid gaps between brushes when moving fast
			...
			int minDistance = paintEngine.customBrush.width;
			...
			{
				pixelCount++;
				if(Vector2.Distance(new Vector2(x0,y0),end) < (minDistance*0.9f))
				{
					loop = false;
					break;
				}
		...
		public static void DrawCustomBrush2(int px,int py,AdvancedMobilePaint paintEngine)
		{
			...
			for (int y = 0; y < paintEngine.customBrushHeight; y++) 
			{
				for (int x = 0; x < paintEngine.customBrushWidth; x++) 
				{
					...
					{
						if(paintEngine.maskPixels[pixel+3]!=0) skip=true;
					}
