...
//				if (hitColorA==0) return;
				if (hitColorA==255) 
				{
					Debug.Log("izaso iz FloodFill");
					return;		//FIXME Cackano!
				}
			}
...
//				if (hitColorR<=50 && hitColorG<=50 && hitColorB<=50 && hitColorA>0.8f) return; //FIXME vratiti vrednosti na 0 kako bi flood fill bojio sve osim PURE BLACK
			}
...
//					    && paintEngine.pixels[pixel+3]<=hitColorA+0.7f) //FIXME vrati cackano
					{
						fillPointX.Enqueue(ptsx);
						fillPointY.Enqueue(ptsy-1);
//						paintEngine.DrawPoint(pixel);
				...
				if (ptsx+1<paintEngine.texWidth)
				{
					...
					if (paintEngine.pixels[pixel+0]==hitColorR 
					    ...
					    && paintEngine.pixels[pixel+2]==hitColorB 
					    && paintEngine.pixels[pixel+3]==hitColorA)
//						&& paintEngine.pixels[pixel+3]<=hitColorA+0.5f) //FIXME vrati cackano
					{
						fillPointX.Enqueue(ptsx+1);
						fillPointY.Enqueue(ptsy);
//						paintEngine.DrawPoint(pixel);
