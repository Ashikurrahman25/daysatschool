...
//					Debug.Log(" Player prefs: " + playerPrefs[i].value.stringValue + " Type: " + playerPrefs[i].value.stringValue.GetType() );
					break;
				}
				GUILayout.FlexibleSpace();
	...
	private void GetPrefKeysWindows() {
		...
		string regKey;
		if(Application.unityVersion == "5.5.2f1")
			regKey = @"SOFTWARE\Unity\UnityEditor\" + PlayerSettings.companyName + @"\" + PlayerSettings.productName;
		else
			regKey = @"Software\" + PlayerSettings.companyName + @"\" + PlayerSettings.productName;	// Kod starijih verzija Unity-a, registri su na drugoj putanji
		...
		foreach (string subkeyName in key.GetValueNames()) {
			...
			if (!float.IsNaN (PlayerPrefs.GetFloat (keyName, float.NaN))) {
				...
				newType = "string";
					val = PlayerPrefs.GetString(subkeyName);
			}
